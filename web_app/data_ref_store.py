...
def load_data_ref_rows(
    ...
    try:
        ...
        loaded = rows_from_data_ref_document(document, limit=limit)
...
def rows_from_data_ref_document(document: dict[str, Any], limit: int | None = None) -> dict[str, Any]:
    ...
    row_count = _positive_int(document.get("row_count"), len(rows))
    columns = _string_list(document.get("columns")) or _rows_columns(rows)
