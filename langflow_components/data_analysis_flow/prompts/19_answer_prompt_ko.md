# 19 Answer Prompt Builder - 한글 프롬프트

`19 Answer Prompt Builder`에서 사용할 수 있는 한글 지시문 버전이다.
...
`INPUT 계획`과 `INPUT계획`, `OUT 계획`과 `OUT계획`처럼 공백만 다른 수량 컬럼명 차이는 source 컬럼 오류나 사용자 수정 요청으로 설명하지 마세요. metadata/source summary에 보이는 실제 컬럼명을 기준으로 처리된 것으로 설명하세요.
