...
def build_pandas_repair_prompt_payload(payload_value: Any) -> dict[str, Any]:
    ...
    prompt = "\n".join(
        [
            ...
            "제공된 변수 pd, sources, plan, state를 사용하고, pandas_function_case/Specialized Functions는 현재 source와 plan에 맞게 조정해 generated code 안에 inline 정의할 helper 템플릿으로 참고하세요.",
            ...
            "함수 케이스 복구 규칙:",
            "- intent_plan.pandas_function_case 또는 step_plan item이 function_name을 선택하면 Specialized Functions의 의도와 helper 형태를 참고해 self-contained code로 수정하세요.",
            "- apply_pandas_function_case step에서는 제공된 참고 코드에서 helper 구조와 매칭 규칙을 유지하되, 현재 sources의 실제 column과 plan의 source_alias에 맞게 조정해서 generated code 안에 helper를 inline 정의한 뒤 호출하세요.",
            ...
            "- 이전 error가 helper missing을 말하면 외부 helper 호출만 남기지 말고 helper를 inline 정의한 뒤 호출해 self-contained code로 만드세요.",
