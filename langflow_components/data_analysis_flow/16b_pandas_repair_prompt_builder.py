...
def build_pandas_repair_prompt_payload(payload_value: Any) -> dict[str, Any]:
    ...
    prompt = "\n".join(
        [
            ...
            "Generate corrected Python pandas code that uses only the provided variables: pd, sources, plan, state, and helper functions selected by pandas_function_case/Specialized Functions.",
            ...
            "plan is already the normalized intent plan itself. Never create or reference plan['intent_plan'].",
            ...
            "Function-case repair rules:",
            "- If intent_plan.pandas_function_case or any step_plan item selects a function_name, use the Specialized Functions intent and helper shape when repairing the code.",
            "- For an apply_pandas_function_case step, either call the selected helper as function_name(input_text, sources[source_alias]) or define the helper inline from the provided reference and then call it.",
            "- Read helper input_text from the current step.get('input_text') or plan.get('pandas_function_case', {}).get('input_text'); plan['intent_plan'] does not exist.",
            "- Use positional arguments for selected helper calls when possible. Do not invent keyword names such as source_df= unless the helper signature explicitly documents that keyword.",
            "- Do not bypass a selected function case with unrelated simple filters when Specialized Functions describes richer matching logic.",
            "- For apply_pandas_function_case followed by aggregate/rank/detail steps, first call the helper to create the filtered DataFrame, then aggregate/rank/detail from that helper output.",
            "- If the previous error says a helper is missing, make the repaired code self-contained by defining the helper inline before calling it, or call the helper only when it is available in the executor.",
