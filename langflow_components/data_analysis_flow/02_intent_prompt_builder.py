...
def build_intent_prompt_payload(payload_value: Any) -> dict[str, Any]:
    ...
    prompt = "\n".join(
        [
            ...
            json.dumps(
                {
                    ...
                    "filter_scopes": [
                        {
                            "scope_key": "short semantic scope key such as da_scope, input_production_scope, all_process_wip_scope",
                            "label": "user-facing scope label",
                            "source_aliases": ["source aliases this scope applies to"],
                            "dataset_keys": ["optional dataset keys this scope applies to"],
                            "dataset_families": ["optional dataset families this scope applies to"],
                            "filters": [{"field": "metadata filter field", "op": "eq|in|not_empty|empty|starts_with|last_char_in", "value": "optional", "values": []}],
                            "blocked_filter_fields": ["optional filter fields that must not be inferred for this scope"],
                            "output_prefix": "optional metric prefix such as DA, WB, INPUT, BG1",
                            "scope_output_column": "optional user-facing label column such as OPER_GROUP or PRODUCT_GROUP",
                        }
                    ],
                    ...
                    "product_attribute_resolution": [
                        {
                            "resolver_key": "product_attribute_resolvers key from metadata",
                            "source_alias": "source alias to apply this resolver to",
                            "tokens": ["unlabeled product attribute tokens from the question"],
                            "attribute_columns": ["copy resolver attribute_columns/output_filter_columns from metadata"],
                            "prefix_match_columns": {"column": {"pattern": "metadata pattern", "match": "starts_with"}},
                            "apply_stage": "after_retrieval_filters_before_aggregation",
                            "on_no_match": "return_empty_result_with_reason",
                        }
                    ],
                    "retrieval_jobs": [
                        {
            ...
            "- product_attribute_resolvers are pandas-stage matching hints for already retrieved runtime sources. Do not create retrieval_jobs, datasets, or required_columns for a product master/lookup dataset from product_attribute_resolvers.",
            "- When only product_attribute_resolvers can resolve a product expression, choose datasets from the requested metrics/quantities and leave the token-to-column value matching to the pandas generation stage.",
            "- For product_attribute_resolvers, do not turn unlabeled product tokens into retrieval_jobs[].filters just because a metadata filter field exists. If the user says values like '512G LPDDR5 L-269 제품' without naming a column, leave those token matches to pandas_logic_hints. Keep only DATE/process filters and filters from named product_terms/status_terms/quantity_terms.",
            "- Only create a resolver attribute retrieval filter when the user explicitly names the filter field or source column, such as 'MCP_NO가 ...', 'MODE가 ...', or 'DEN이 ...'. Otherwise, product attribute values and product-code prefixes must be matched against runtime source DataFrame distinct values in pandas.",
            "- When a product_attribute_resolver is the way to resolve a product expression, return product_attribute_resolution and add a step_plan item with operation='apply_product_attribute_resolver' before aggregation/ranking/join steps that depend on that filtered source.",
            "- product_attribute_resolution must copy resolver columns and prefix rules from metadata. Do not invent columns or prefix rules.",
            "- product_attribute_resolution.tokens must contain only the unlabeled product attribute tokens from the question, excluding trigger words such as 제품/PRODUCT/자재/DEVICE, process words, date words, and metric words.",
            "- If product_attribute_resolution tokens are present and none match runtime source values in pandas, the result must not silently fall back to the unfiltered total; set on_no_match='return_empty_result_with_reason'.",
            "- If a question asks a product condition resolved from product_terms or product_attribute_resolvers plus DA/WB production and WIP together, use production_today and wip_today with the process group filters and analysis_kind=aggregate_join.",
            ...
            "- Separate global filters from source-specific scope filters. Use top-level filters only for conditions that must apply to every retrieval job, such as the same DATE or the same product condition across all sources.",
            "- If a condition applies only to one requested measure/source/scope, put it in that retrieval_jobs[].filters or in filter_scopes with matching source_aliases/dataset_families. Do not put it in top-level filters.",
            "- If the question compares scopes such as A 대비 B, DA와 WB 각각, or metric A plus all-process metric B, preserve each scope separately. Use distinct source aliases and filter_scopes, or distinct step_plan items with scope labels/output prefixes. Do not merge the scope filters into one union filter unless the final answer is explicitly 공정별/by process.",
            "- Quantity term conditions from domain metadata, such as a quantity_terms entry with dataset_family and condition, apply only to the dataset family/source that calculates that quantity. They are not global filters for unrelated metrics.",
            "- Product term conditions from product_terms are filters, not grouping grain. When a product term is used only as a condition, use a user-facing scope label such as PRODUCT_GROUP rather than showing the raw condition column/value in the final output.",
            "- For source scopes described as 전체 공정, 전 공정, all process, or no process restriction, explicitly block OPER_NAME/process filters for that source with filter_scopes[].blocked_filter_fields or the retrieval job's filter_policy.blocked_filter_fields.",
...
def _metadata_summary(metadata: dict[str, Any], request_date: str) -> dict[str, Any]:
    ...
    return {
        ...
        "product_attribute_resolvers": domain.get("product_attribute_resolvers", {}),
