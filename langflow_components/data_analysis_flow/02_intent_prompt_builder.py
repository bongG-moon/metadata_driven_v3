...
def build_intent_prompt_payload(payload_value: Any, specialized_prompt_text: Any = "") -> dict[str, Any]:
    ...
    prompt = "\n".join(
        [
            ...
            "- LPDDR5 48G CP처럼 등록 product_terms가 아닌 여러 제품 속성 token이 제품 조건으로 나열되면 MODE/DEN/PKG 같은 retrieval filter를 임의로 만들지 말고 component_token_product_lookup function case를 먼저 계획하세요.",
            "- POP, MOBILE, HBM처럼 등록 product_terms로 정의된 제품군은 function case가 아니라 해당 product_terms의 metadata-backed filter 조건을 사용하세요.",
