...
def build_intent_prompt_payload(payload_value: Any, specialized_prompt_text: Any = "") -> dict[str, Any]:
    ...
    prompt = "\n".join(
        [
            ...
            "- pandas_function_case의 input_text에는 해당 helper가 판단해야 하는 핵심 표현을 빠뜨리지 말고 포함하세요.",
            "- pandas_function_case의 input_text에는 날짜/시점, source scope, metric/동사 표현처럼 helper 입력이 아닌 문구는 넣지 마세요.",
            "- 사용자가 표준 field 이름과 연산자/값을 명시하면 절차형 helper로 바꾸지 말고 retrieval_jobs[].filters 또는 plan.filters에 해당 field filter를 보존하세요.",
            "- 생산량만 묻는 질문에는 production 계열 dataset만 선택하고 WIP/재공 dataset을 추가하지 마세요. 재공만 묻는 질문에는 wip 계열 dataset만 선택하고 production dataset을 추가하지 마세요. 두 metric을 함께 묻는 경우에만 production+wip multi-source 계획을 만드세요.",
            ...
            "- 같은 의미의 standard/alias column을 동시에 output_columns에 넣지 마세요. 예: DEN과 DENSITY를 함께 반환하지 말고 standard column인 DEN만 사용하세요. PRODUCTION이 있으면 TOTAL_PRODUCTION 같은 중복 metric alias를 만들지 마세요.",
