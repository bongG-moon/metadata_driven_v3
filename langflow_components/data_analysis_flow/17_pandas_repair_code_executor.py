# 파일 설명: 17 Pandas Repair Code Executor Langflow custom component 파일입니다.
# 흐름 역할: 16B repair prompt 이후 LLM이 만든 pandas JSON/code를 독립 실행합니다.
...
# 컴포넌트 설명: 17 Pandas Repair Code Executor
# Langflow 표시 설명: 16B repair LLM 응답을 파싱하고 안전성 검사 후 runtime source DataFrame 위에서 실행합니다.
class PandasRepairCodeExecutor(Component):

    display_name = "17 Pandas Repair Code Executor"
    description = "16B repair LLM 응답을 파싱하고 안전성 검사 후 runtime source DataFrame 위에서 실행합니다."
    inputs = [
    ...
    # 처리 역할: 16B repair LLM 응답을 파싱하고 안전성 검사 후 runtime source DataFrame 위에서 실행합니다.
