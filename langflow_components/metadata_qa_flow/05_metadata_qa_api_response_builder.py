# 파일 설명: 05 Metadata QA API Response Builder Langflow custom component 파일입니다.
# 흐름 역할: metadata QA 최종 payload를 web/API client가 쓰기 쉬운 compact JSON 응답으로 투영합니다.
...
# 처리 역할: metadata QA 최종 payload를 web/API client가 쓰기 쉬운 compact JSON 응답으로 투영합니다.
...
# 컴포넌트 설명: 05 Metadata QA API Response Builder
# Langflow 표시 설명: metadata QA 최종 payload를 web/API client가 쓰기 쉬운 compact JSON 응답으로 투영합니다.
class MainFlowApiResponseBuilder(Component):

    display_name = "05 Metadata QA API Response Builder"
    description = "metadata QA 최종 payload를 web/API client가 쓰기 쉬운 compact JSON 응답으로 투영합니다."
    icon = "Braces"
    name = "MainFlowApiResponseBuilder"

    inputs = [
        DataInput(name="payload", display_name="Payload", info="Payload output from 03 Metadata QA Response Builder.", input_types=["Data", "JSON"], required=True),
    ]
