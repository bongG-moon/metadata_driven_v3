# 파일 설명: 00 Report Request Loader Langflow custom component 파일입니다.
# 흐름 역할: 리포트 생성 질문과 session/state를 compact report request payload로 정리합니다.
...
# 처리 역할: 리포트 생성 질문과 session/state를 compact report request payload로 정리합니다.
# Langflow wrapper와 단위 테스트가 같은 로직을 재사용할 수 있도록 순수 dict/string 결과를 만듭니다.
def build_report_request(question: str, session_id: str = "", state: Any = None) -> dict[str, Any]:
...
# 컴포넌트 설명: 00 Report Request Loader
# Langflow 표시 설명: 리포트 생성 질문과 session/state를 compact report request payload로 정리합니다.
class ReportRequestLoader(Component):

    display_name = "00 Report Request Loader"
    description = "리포트 생성 질문과 session/state를 compact report request payload로 정리합니다."
    icon = "FileText"
    inputs = [
