# 파일 설명: 08 Domain Authoring Response Builder Langflow custom component 파일입니다.
# 흐름 역할: domain metadata 등록 결과를 사용자가 읽기 좋은 한국어 Playground/API 응답으로 만듭니다.
...
# 처리 역할: domain metadata 등록 결과를 사용자가 읽기 좋은 한국어 Playground/API 응답으로 만듭니다.
# Langflow wrapper와 단위 테스트가 같은 로직을 재사용할 수 있도록 순수 dict/string 결과를 만듭니다.
def build_domain_authoring_response(payload_value: Any) -> dict[str, Any]:
...
def _message(payload: dict[str, Any], review: dict[str, Any], write_result: dict[str, Any]) -> str:
    lines = ["### Domain metadata authoring result"]
    if write_result.get("status") == "ok":
        saved = ", ".join(f"{item.get('section')}/{item.get('key')}" for item in write_result.get("saved_items", []))
    ...
    if payload.get("existing_matches") or payload.get("conflict_warnings"):
        ...
        for item in payload.get("existing_matches", [])[:5]:
            lines.append(f"- {item.get('reason')} ({(item.get('existing') or {}).get('section')}/{(item.get('existing') or {}).get('key')})")
        for item in payload.get("conflict_warnings", [])[:5]:
            lines.append(f"- 경고: {item.get('reason')} ({(item.get('existing') or {}).get('section')}/{(item.get('existing') or {}).get('key')})")
    if payload.get("items"):
        lines.append("")
        lines.append("생성된 item:")
        for item in payload.get("items", [])[:10]:
            lines.append(f"- {item.get('section')}/{item.get('key')}")
    return "\n".join(lines)
...
# 컴포넌트 설명: 08 Domain Authoring Response Builder
# Langflow 표시 설명: domain metadata 등록 결과를 사용자가 읽기 좋은 한국어 Playground/API 응답으로 만듭니다.
class DomainAuthoringResponseBuilder(Component):

    display_name = "08 Domain Authoring Response Builder"
    description = "domain metadata 등록 결과를 사용자가 읽기 좋은 한국어 Playground/API 응답으로 만듭니다."
    ...
    def build_api_response(self) -> Data:
        result = build_domain_authoring_response(getattr(self, "payload", None))
    ...
    def build_message(self) -> Message:
        return Message(text=build_domain_authoring_response(getattr(self, "payload", None))["message"])
