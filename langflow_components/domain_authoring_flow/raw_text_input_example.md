...
이 케이스는 match_product_tokens helper를 사용해서 조회된 제품 데이터의 TECH, DEN/DENSITY, MODE, ORG, PKG1/PKG_TYPE1, PKG2/PKG_TYPE2, LEAD, MCP_NO 값과 입력 토큰을 매칭해 일치하는 제품 row를 찾는 용도야.
helper의 실제 코드 예시는 MongoDB raw text에 넣지 말고 14 Pandas Prompt Builder의 Pandas Function Cases 입력 또는 별도 helper package에서 관리해.
```
<!-- single_pandas_function_case_component_token_product_lookup:end -->

<!-- single_pandas_function_case_lot_hold_complex_lookup:start -->
```text
Lot Hold 복합 조건 조회용 pandas_function_cases 항목을 등록해줘.
key는 lot_hold_complex_lookup이고 function_name은 match_lot_hold_conditions야.
display_name은 Lot Hold 복합 조회, aliases는 Lot Hold 조회, Hold 사유 조회, Lot 상태 조회, input_text는 사용자 질문 전체야.
사용자가 Lot ID, Hold 상태/사유, 공정명, IN_TAT/HOLD_TM 조건을 섞어서 Lot/Hold 목록을 찾으면 이 helper를 쓰도록 저장해줘.
실제 helper 코드는 MongoDB에 저장하지 말고 14 Pandas Prompt Builder의 Pandas Function Cases 입력에서 관리해.
```
<!-- single_pandas_function_case_lot_hold_complex_lookup:end -->
