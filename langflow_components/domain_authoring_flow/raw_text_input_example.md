...
helper의 실제 코드 예시는 MongoDB raw text에 넣지 말고 14 Pandas Prompt Builder / 15 Pandas Code Executor / 17 Pandas Repair Code Executor의 Specialized Functions 입력 또는 별도 helper package에서 관리해.
```
...
생산달성률, 생산달성율, 달성률, 달성율은 단순 metric formula가 아니라 생산 달성율 분석 recipe를 우선 사용해.
달성률을 평균내지 말고, recipe에서 정한 기준으로 실적수량과 계획수량을 먼저 집계한 뒤 비율을 계산해.
```
<!-- single_metric_terms:end -->

<!-- single_recipe_production_achievement_rate_by_target_plan:start -->
```text
특정 일자의 생산달성율 분석 recipe를 등록해줘.
section은 analysis_recipes이고 key는 production_achievement_rate_by_target_plan이야.
display_name은 생산 달성율이고 aliases는 생산달성율, 생산달성률, 달성율, 달성률, 목표대비 실적, 계획대비 실적, 생산 목표 달성율이야.

사용자가 특정 일자의 생산달성율이나 계획 대비 실적을 물으면 production 계열 실적 데이터와 target 계열 Goodocs 목표/계획 데이터를 함께 사용해.
production 데이터는 DATE, MODE, DEN, TECH, ORG, PKG_TYPE1, PKG_TYPE2, LEAD, MCP_NO, OPER, OPER_NAME, OPER_SEQ 기준으로 PRODUCTION을 합산해.
target 데이터는 DATE, MODE, DEN, TECH, ORG, PKG_TYPE1, PKG_TYPE2, LEAD, MCP_NO 기준으로 INPUT 계획과 OUT 계획을 합산해.

두 데이터는 DATE, MODE, DEN, TECH, ORG, PKG_TYPE1, PKG_TYPE2, LEAD, MCP_NO 기준으로 FULL OUTER JOIN해.
DATE 형식이 데이터셋마다 다르면 비교 전에 같은 날짜 형식으로 정규화해.

JOIN 후 production의 OPER 값을 숫자로 해석해서 OPER가 2000보다 작으면 계획수량은 INPUT 계획 * 1000으로 계산하고, OPER가 2000보다 크면 계획수량은 OUT 계획 * 1000으로 계산해.
OPER가 없거나 계획수량이 없거나 계획수량이 0이면 달성율은 임의로 0을 만들지 말고 null로 둬.
생산달성율은 실적수량 / 계획수량으로 계산해.
결과에는 DATE, 제품 기준 컬럼, OPER, OPER_NAME, OPER_SEQ, 실적수량, 계획수량, 생산달성율을 보여줘.
이 블록은 analysis_recipes만 등록하고, production/target table catalog나 공정 그룹은 새로 만들지 마.
```
<!-- single_recipe_production_achievement_rate_by_target_plan:end -->
