...
def build_domain_review_prompt_payload(payload_value: Any) -> dict[str, Any]:
            ...
            json.dumps(
                {
                    ...
                    "item_reviews": [{"section": "section", "key": "key", "decision": "pass | needs_fix", "reason": "Korean reason"}],
                },
                ensure_ascii=False,
                indent=2,
            ),
    ...
    return {"prompt": prompt, "payload": payload, "prompt_type": "domain_save_review"}
...
class DomainReviewPromptBuilder(Component):
    display_name = "06 Domain Review Prompt Builder"
    description = "Builds the final Gemini/LLM review prompt before saving domain metadata."
    ...
    def build_prompt(self) -> Message:
        prompt_payload = build_domain_review_prompt_payload(getattr(self, "payload", None))
    ...
    def build_prompt_payload(self) -> Data:
        return Data(data=build_domain_review_prompt_payload(getattr(self, "payload", None)))
