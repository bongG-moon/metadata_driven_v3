# 파일 설명: 06 Domain Review Variables Builder Langflow custom component 파일입니다.
# 흐름 역할: Domain Review Prompt Template에 넣을 review_input_json 변수를 준비합니다.
...
# 처리 역할: Domain Review Prompt Template에 넣을 review_input_json 변수를 준비합니다.
# Langflow wrapper와 단위 테스트가 같은 로직을 재사용할 수 있도록 순수 dict/string 결과를 만듭니다.
def build_domain_review_prompt_variables(payload_value: Any) -> dict[str, Any]:
    ...
    return {
        "prompt_type": "domain_save_review",
        "payload": payload,
...
# Langflow 표시 설명: Domain Review Prompt Template에 넣을 review_input_json 변수를 준비합니다.
class DomainReviewVariablesBuilder(Component):

    display_name = "06 Domain Review Variables Builder"
    description = "Domain Review Prompt Template에 넣을 review_input_json 변수를 준비합니다."
    ...
    def build_review_input_json(self) -> Message:
        variables = build_domain_review_prompt_variables(getattr(self, "payload", None))
