...
quantity_terms에서는 dataset_family 또는 quantity_column이 명확하면 dataset_key를 필수로 요구하지 마세요.
중복 제거 수량 term은 aggregation='nunique'와 quantity_column이 있으면 충분합니다. output_column은 생성 가능하므로 사람이 직접 제공하지 않았다는 이유만으로 막지 마세요.
