...
dataset_key/name is user-owned. If no dataset_key/name was explicitly provided, request it.
When table_name/query_template and columns identify the dataset, do not block only because db_key, default_detail_columns, or primary_quantity_column is absent.
For oracle/datalake draft catalog items, table_name+columns is enough to save when query_template was not provided.
