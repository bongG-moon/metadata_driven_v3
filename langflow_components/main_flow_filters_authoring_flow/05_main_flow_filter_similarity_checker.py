# 파일 설명: 05 Main Flow Filter Similarity Checker Langflow custom component 파일입니다.
# 흐름 역할: 저장 전 같은 key, alias 겹침, column 후보 겹침 같은 filter metadata 충돌 가능성을 찾습니다.
...
# 처리 역할: 저장 전 같은 key, alias 겹침, column 후보 겹침 같은 filter metadata 충돌 가능성을 찾습니다.
...
# 컴포넌트 설명: 05 Main Flow Filter Similarity Checker
# Langflow 표시 설명: 저장 전 같은 key, alias 겹침, column 후보 겹침 같은 filter metadata 충돌 가능성을 찾습니다.
class MainFlowFilterSimilarityChecker(Component):

    display_name = "05 Main Flow Filter Similarity Checker"
    description = "저장 전 같은 key, alias 겹침, column 후보 겹침 같은 filter metadata 충돌 가능성을 찾습니다."
    inputs = [
