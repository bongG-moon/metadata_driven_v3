# 파일 설명: 02 Main Flow Filter Text Refinement Normalizer Langflow custom component 파일입니다.
# 흐름 역할: filter 설명 정제 LLM JSON을 compact filter authoring payload에 반영합니다.
...
# 처리 역할: filter 설명 정제 LLM JSON을 compact filter authoring payload에 반영합니다.
...
# 컴포넌트 설명: 02 Main Flow Filter Text Refinement Normalizer
# Langflow 표시 설명: filter 설명 정제 LLM JSON을 compact filter authoring payload에 반영합니다.
class MainFlowFilterTextRefinementNormalizer(Component):

    display_name = "02 Main Flow Filter Text Refinement Normalizer"
    description = "filter 설명 정제 LLM JSON을 compact filter authoring payload에 반영합니다."
    inputs = [
